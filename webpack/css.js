...
module.exports = source => {
	...
	parser.walk(tree, node => {
		...
		if(read.startsWith('assets:'))node.value.value = JSON.stringify('data:application/octet-stream;base64,' + fs.readFileSync(path.join(__dirname, '..', 'sploit', 'libs', read.substr(7)), 'base64'));
	});
